...
int Training(const string pos_images_path, const string neg_images_path, const Size& image_size)
{
	if (!file_exists(TRAINED_SVM)) {
		...
		load_images(pos_images_path, pos_lst);
		labels.assign(pos_lst.size(), +1);

		load_images(neg_images_path, full_neg_lst);
...
//		for (int iRect=0; iRect<locationCount; iRect++)
